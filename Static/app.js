...
const activeThemeModule = (() => {
  const loader = window.WeldModules;
  if (!loader || typeof loader.has !== "function") return null;
  try {
    if (loader.has("runtime/theme")) {
      return loader.use("runtime/theme");
    }
  } catch (error) {
    console.warn("runtime/theme module unavailable.", error);
  }
  return null;
})();

const normalizeTheme =
  activeThemeModule?.normalizeTheme ||
...
const applyTheme =
  activeThemeModule?.applyTheme
    ? function applyTheme(theme) {
        return activeThemeModule.applyTheme(theme, state);
      }
    : function applyTheme(theme) {
...
if (activeThemeModule?.initializeTheme) {
  try {
    activeThemeModule.initializeTheme(state);
  } catch (error) {
    console.warn("runtime/theme initializeTheme failed; falling back to inline theme logic.", error);
...
const activeServiceWrapperModule = (() => {
  const loader = window.WeldModules;
  if (!loader || typeof loader.has !== "function") return null;
  try {
    if (loader.has("runtime/serviceWrappers")) {
      return loader.use("runtime/serviceWrappers");
    }
  } catch (error) {
    console.warn("runtime/serviceWrappers module unavailable.", error);
  }
  return null;
})();
...
const activeAchievementsModule = (() => {
  ...
  if (!loader || typeof loader.has !== "function") return null;
  try {
    if (loader.has("runtime/achievements")) {
      return loader.use("runtime/achievements");
    }
  } catch (error) {
...
const queueBadgeAchievements =
  achievementsApiModule && typeof achievementsApiModule.queueBadgeAchievements === "function"
    ...
    : function queueBadgeAchievements(badgeInput) {
        if (Array.isArray(badgeInput) && badgeInput.length > 0) {
...
const renderBadgeLabOrb =
  activeAchievementsModule && typeof activeAchievementsModule.renderBadgeLabOrb === "function"
    ? function renderBadgeLabOrbProxy(badge, options) {
        return activeAchievementsModule.renderBadgeLabOrb(badge, options);
      }
    : function renderBadgeLabOrbFallback() {
        return null;
      };
...
const ensureWeldAchievementsBinding = () => {
  ...
  fallbackWeldAchievements.queueBadgeUnlocks = function fallbackQueueBadgeUnlocks(badges, options) {
    queueBadgeAchievements(badges, options);
  };
  fallbackWeldAchievements.queueBadgeAchievements = function fallbackQueueBadgeAchievements(badges, options) {
    queueBadgeAchievements(badges, options);
  };
  if (typeof fallbackWeldAchievements.isActive !== "function") {
    fallbackWeldAchievements.isActive = function fallbackIsActive() {
      return false;
    };
  }
