...
| 2025-11-15 | Quest data modularization | Phase C - Data & State Deduplication | `node Static/tools/state-vm-smoke.js` + manual hub/catalog check; quests now sourced via `data/quests/*.js` modules. |
...
| 2025-11-15 | Badge metadata module | Phase C - Data & State Deduplication | `node Static/tools/state-vm-smoke.js`; badge tones/icon paths moved into `data/catalog/badgeMeta.js`, with app + customer hub now consuming the module (manual UI sweep pending). |
| 2025-11-15 | Quest config module | Phase C - Data & State Deduplication | `node Static/tools/state-vm-smoke.js`; quest difficulty ordering now lives in `data/quests/config.js`, with `app.js`/`util.js` using the module (hub quests QA pending). |
| 2025-11-15 | Global nav metadata hookup | Phase C - Data & State Deduplication | `node Static/tools/state-vm-smoke.js`; `components/globalNav.js` now resolves enums/routes via `WeldModules` (needs manual click-through in browser). |
| 2025-11-15 | Legacy badge block removed | Phase C - Data & State Deduplication | `node Static/tools/state-vm-smoke.js`; stripped badge/quest metadata from `data/app/__legacy__.js` now that modules power all consumers (prep for validation script). |
